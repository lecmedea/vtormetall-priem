...
export default async function ServicePage({ params }: Props) {
  ...
  const path = `/uslugi/${item.slug}`;
  const photo = servicePhotos[item.slug];
  return <><JsonLd data={[breadcrumbSchema([["Главная", "/"], ["Услуги", "/uslugi"], [item.title, path]]), serviceSchema({ name: `${item.title} металлолома в Москве`, description: item.lead, path })]} /><Header /><main><PageHero eyebrow="Услуга" title={item.title} accent="по делу" lead={item.lead} ctaHref="#request" ctaLabel="Обсудить задачу" />
    ...
    <section className="section shell image-section"><ImageBrief title={item.title} src={photo.src} alt={photo.alt} /></section>
