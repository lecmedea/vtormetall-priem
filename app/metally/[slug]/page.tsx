...
export default async function MaterialPage({ params }: Props) {
  ...
  const item = materialCards.find((entry) => entry.slug === slug);
  if (!item) notFound();
  return <><Header /><main><PageHero eyebrow={`Приём · ${item.mark}`} title={`Сдать ${item.title.toLowerCase()}`} accent="выгодно" lead={item.intro} ctaHref="#request" ctaLabel="Узнать условия" index={`${item.mark} / METAL`} />
    ...
    <section className="section shell image-section"><ImageBrief title={`Реальный лом: ${item.title}`} prompt={`Фотореалистичная рекламная фотография реальной партии лома категории «${item.title}»: ${item.examples}, материал аккуратно отсортирован на российском пункте приёма, видна фактура и состояние поверхности, нейтральный промышленный фон, графитовая палитра с кислотно-зелёным акцентом, честный документальный свет, без текста, ценников и логотипов, горизонтальный формат 16:9.`} /></section>
